...
@section('title', 'Thêm shortcode')
@section('page-title', 'Thêm shortcode')

@section('content')
    {{-- Header --}}
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Thêm shortcode</h2>
            <p class="text-sm text-gray-500 mt-1">Tạo shortcode tùy chỉnh mới</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('admin.shortcodes.store') }}" method="POST">
            @csrf
            ...
            <div class="mt-6">
                ...
                <textarea name="content" id="content" rows="10"
                          ...
                          placeholder="Nhập nội dung HTML cho shortcode">{{ old('content') }}</textarea>
                @error('content')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6">
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
                          placeholder="Mô tả ngắn về shortcode">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-8 flex items-center gap-3">
                <button type="submit"
                    ...
                    <i class="fas fa-save mr-2"></i>Lưu shortcode
                </button>
