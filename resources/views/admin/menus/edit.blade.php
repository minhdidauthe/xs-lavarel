...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Sửa menu</h2>
            <p class="text-sm text-gray-500 mt-1">Chỉnh sửa: <strong>{{ $menu->title }}</strong></p>
        </div>
        ...
        <form action="{{ route('admin.menus.update', $menu) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="css_class" id="css_class" value="{{ old('css_class', $menu->css_class) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 font-mono"
                           placeholder="vd: vip">
                ...
                <div>
                    ...
                    <input type="text" name="match_pattern" id="match_pattern" value="{{ old('match_pattern', $menu->match_pattern) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 font-mono"
                ...
                <div>
                    ...
                    <select name="target" id="target"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500">
                        <option value="_self" {{ old('target', $menu->target) === '_self' ? 'selected' : '' }}>Cùng tab (_self)</option>
                        <option value="_blank" {{ old('target', $menu->target) === '_blank' ? 'selected' : '' }}>Tab mới (_blank)</option>
                    </select>
                </div>
            ...
            <div class="mt-8 flex items-center gap-3">
                <button type="submit"
                    ...
                    <i class="fas fa-save mr-2"></i>Cập nhật menu
                </button>
