...
@section('title', $category->name . ' - Blog - SOICAU7777.CLICK')
...
<div class="max-w-7xl mx-auto px-4 py-10">
    ...
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-black text-white uppercase tracking-tight">
            <i class="fas fa-folder-open text-red-500 mr-2"></i>{{ $category->name }}
        </h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @forelse($posts as $post)
            <a href="{{ route('blog.show', $post->slug) }}" class="glass-card rounded-2xl overflow-hidden group">
                ...
                @endif
                <div class="p-4">
                    <h3 class="text-sm font-bold text-white group-hover:text-red-400 transition-colors line-clamp-2">{{ $post->title }}</h3>
                    @if($post->excerpt)
                    ...
                    @if($post->tags->count())
                        <div class="flex flex-wrap gap-1 mt-3">
                            @foreach($post->tags->take(3) as $tag)
                                <span class="text-[9px] bg-white/5 text-gray-500 px-2 py-0.5 rounded-full">#{{ $tag->name }}</span>
                            @endforeach
                        </div>
                    @endif
                </div>
